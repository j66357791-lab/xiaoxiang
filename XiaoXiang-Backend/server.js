...
// 注意：Multer 中间件配置可以在具体路由文件里使用，这里主要负责静态文件
...
// 👇 修复点 1：提供静态文件访问 (让前端能访问上传的图片)
// 必须在路由之前挂载
app.use('/uploads', express.static(path.join(process.cwd(), 'uploads')));
console.log('✅ 静态文件服务已挂载: /uploads');
...
// 👇 修复点 2：删除了多余的 Multer 路由挂载
// 之前的代码: app.post('/api/admin/job', upload.array('contentImages'), adminRoutes);
// 现在的任务发布使用 Base64 方式，在 adminRoutes 内部处理，不需要中间件拦截。
// 如果未来要改成文件上传方式，请恢复此行并调整 adminRoutes 逻辑。
...
const startServer = async () => {
  try {
    ...
    const server = app.listen(PORT, '0.0.0.0', () => {
      ...
      console.log(`   📦📦 订单接口: http://localhost:${PORT}/api/orders`);
