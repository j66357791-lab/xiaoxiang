...
import adminRoutes from './routes/admin.js';     // 👈 新增：管理员路由
import jobRoutes from './routes/jobs.js';       // 👈 新增：兼职任务路由
import orderRoutes from './routes/orders.js';   // 👈 新增：订单路由
...
const connectDB = async () => {
  try {
    // 支持多种环境变量名称
    const mongoUri = process.env.MONGODB_URI="mongodb+srv://j66357791_db_user:hjh628727@cluster0.oiwbvje.mongodb.net/invest-v5?retryWrites=true&w=majority";
    ...
    // 修复点：移除已废弃的 MongoDB 选项
    const options = {
      // useNewUrlParser: true,    // 已废弃，移除
      // useUnifiedTopology: true, // 已废弃，移除
    ...
    console.error('\n💡💡 建议解决方案:');
    console.error('   1. 检查 MONGODB_URI 环境变量是否正确');
    console.error('   2. 确认数据库服务器正在运行');
    console.error('   3. 检查网络连接和防火墙设置');
    console.error('   4. 验证数据库用户名和密码');
...
// 404 处理 - 修复点：移除了通配符 '*' 参数
// Express 5.x 中 app.use('*', ...) 语法已废弃，改为直接使用函数
app.use((req, res) => {
  res.status(404).json({ 
    success: false, 
    message: 'API 路由不存在',
