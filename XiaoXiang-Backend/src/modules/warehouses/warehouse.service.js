...
export class WarehouseService {
  ...
  static async getAllWarehouses(query = {}) {
    ...
    if (query.province) {
      filter['address.province'] = query.province;
    }
  ...
  static async createWarehouse(data) {
    ...
    const warehouse = await Warehouse.create({
      ...
      pickupConfig: {
        enabled: data.pickupEnabled ?? false,
  ...
  static async updateWarehouse(id, data) {
    ...
    // 更新上门回收配置
    if (data.pickupEnabled !== undefined) {
      warehouse.pickupConfig = warehouse.pickupConfig || {};
      warehouse.pickupConfig.enabled = data.pickupEnabled;
    }
    if (data.serviceCities !== undefined) {
      warehouse.pickupConfig = warehouse.pickupConfig || {};
      warehouse.pickupConfig.serviceCities = data.serviceCities;
    }
    if (data.pickupNotice !== undefined) {
      warehouse.pickupConfig = warehouse.pickupConfig || {};
      warehouse.pickupConfig.notice = data.pickupNotice;
    }
   ...
   * 🆕 按城市检查上门回收服务
   */
  static async checkPickupByCity(city) {
    if (!city) {
      throw new BadRequestError('请选择城市');
    }
    
    console.log('[WarehouseService] 📍 检查上门回收服务:', city);
    ...
    const warehouses = await Warehouse.find({
      ...
      $or: [
        { 'pickupConfig.serviceCities': city },
        { 'address.city': city },
      ],
    }).lean();
    
    const available = warehouses.length > 0;
    
    console.log('[WarehouseService] ✅ 找到', warehouses.length, '个支持该城市的仓库');
  ...
  static async checkPickupAvailability(longitude, latitude) {
    if (!longitude || !latitude) {
      throw new BadRequestError('缺少位置信息');
    }
    ...
    const results = warehouses.map(wh => {
      ...
      return {
        ...
        inService: distance <= (wh.service?.pickupRadius || 10),
      };
    });
