 ...
 * 
 * 管理员接口说明：
 * - getTeamLeaderList() - 获取团队长列表
 * - getTeamLeaderDetail(leaderId) - 获取团队长详情
 * - distributeWeeklyRewards() - 发放周奖励
 * - distributeMonthlyRewards() - 发放月奖励
 * - distributeYearlyRewards() - 发放年终奖励
 * - manualUpgradeRank() - 手动升级团队长
 * - manualDowngradeRank() - 手动降级团队长
 * - recalculateTeamStats() - 重新计算团队数据
 */
import User, { 
  TEAM_LEADER_RANKS, 
  NEWBIE_REWARDS, 
...
export class UserServiceTeam {
  ...
  static async grantPaymentReward(userId) {
    ...
    user.newbieRewards.totalEarned += amount;
    user.pendingEarnings += amount;
    user.hasValidPayment = true; // 标记已绑定收款方式
    await user.save();
   ...
   * @param {ObjectId} workerId - 完成订单的用户ID
   * @param {ObjectId} orderId - 订单ID
  ...
  static async getTeamList(userId, type, keyword) {
    ...
    const selectFields = 'name email avatar createdAt balance isValidMember kycStatus personalOrderCount';
  ...
  static async getMyTeam(userId) {
    const members = await User.find({ inviterId: userId })
      .select('name email avatar createdAt isValidMember personalOrderCount agentRank')
      .sort({ createdAt: -1 });
  ...
  /**
   * [管理员] 发放周奖励
   * @param {ObjectId} leaderId - 团队长ID（可选，不传则发放所有）
  ...
  /**
   * [管理员] 发放月奖励
   * @param {ObjectId} leaderId - 团队长ID（可选）
   * @param {Boolean} dryRun - 是否为预览模式
   */
  ...
  /**
   * [管理员] 发放年终奖励
   * @param {Array} rewards - 奖励列表 [{ leaderId, amount }]
   * @param {Boolean} dryRun - 是否为预览模式
   */
