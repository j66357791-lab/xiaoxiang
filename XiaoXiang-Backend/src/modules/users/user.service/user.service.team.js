...
export class UserServiceTeam {
   ...
   * 有效成员 = 实名认证 + 完成首单
   */
  static async checkIsValidMember(userId) {
    const user = await User.findById(userId);
    if (!user) return false;
    
    const isKycVerified = user.kycStatus === 'Verified';
    const hasFirstOrder = (user.personalOrderCount || 0) >= 1;
    
    return isKycVerified && hasFirstOrder;
  }

  /**
   * 更新用户的有效成员状态
   */
  static async updateValidMemberStatus(userId) {
    const user = await User.findById(userId);
    if (!user) return;
    
    const wasValid = user.isValidMember;
    const isValid = await this.checkIsValidMember(userId);
    ...
    if (isValid && !wasValid) {
      ...
      await this._unfreezeRewards(userId);
      
      console.log(`[Team] 用户 ${userId} 成为有效成员`);
    } else if (!isValid && wasValid) {
      user.isValidMember = false;
      await user.save();
    }
  ...
  static async processOrderRewards(workerId, orderId, orderAmount) {
    ...
    if (!worker) {
      console.log(`[TeamReward] 用户不存在: ${workerId}`);
      return;
    }
    ...
    worker.personalOrderCount = completedOrderCount;
    if (!worker.newbieRewards) {
      worker.newbieRewards = { orderCount: 0, totalEarned: 0 };
    }
    worker.newbieRewards.orderCount = completedOrderCount;
    await worker.save();

    console.log(`[TeamReward] 用户 ${workerId} 完成第 ${completedOrderCount} 单`);

    await this._updateTeamOrderCount(workerId);
    await this.updateValidMemberStatus(workerId);

    if (worker.inviterId) {
      const isKycVerified = worker.kycStatus === 'Verified';
      
      await this.processFriendOrderReward(
        worker.inviterId, 
        workerId, 
        completedOrderCount,
        orderId,
   ...
   * 处理好友订单奖励
   */
  static async processFriendOrderReward(inviterId, friendId, orderNumber, orderId, orderAmount, isFriendKycVerified) {
    ...
    if (isFriendKycVerified) {
      await this._grantReward(inviter, friend, rewardAmount, rewardName, orderId, orderNumber);
      console.log(`[FriendReward] 邀请人 ${inviter._id} 获得${rewardName}: ¥${rewardAmount}`);
    } else {
      await this._freezeReward(inviter, friend, rewardAmount, rewardName, orderId, orderNumber);
      console.log(`[FriendReward] 好友 ${friendId} 未实名，奖励 ¥${rewardAmount} 已冻结`);
    }
  }
   ...
   * 直接发放奖励
   */
  static async _grantReward(inviter, friend, amount, rewardName, orderId, orderNumber) {
    ...
    if (rewardName.includes('首单')) {
      inviter.inviteEarnings.firstOrderBonus = (inviter.inviteEarnings.firstOrderBonus || 0) + amount;
    } else {
      inviter.inviteEarnings.commissionEarned = (inviter.inviteEarnings.commissionEarned || 0) + amount;
    }
    
    inviter.inviteEarnings.totalFromInvite = (inviter.inviteEarnings.totalFromInvite || 0) + amount;
    inviter.pendingEarnings = (inviter.pendingEarnings || 0) + amount;
    await inviter.save();

    const description = `${rewardName} - 好友: ${friend.email || friend.name || friend._id}`;
    await this._createTransaction(inviter._id, amount, 'friend_order_reward', description, orderId, {
      friendId: friend._id,
   ...
   * 冻结奖励（好友未实名）
   */
  static async _freezeReward(inviter, friend, amount, rewardName, orderId, orderNumber) {
    if (!inviter.frozenRewards) {
      inviter.frozenRewards = [];
    }
    
    inviter.frozenRewards.push({
      friendId: friend._id,
      orderId: orderId,
      amount: amount,
      type: rewardName.includes('首单') ? 'first_order' : 'subsequent_order',
      description: `${rewardName} - 好友: ${friend.email || friend.name || friend._id}`,
      orderNumber: orderNumber,
      reason: '好友未实名认证',
      createdAt: new Date()
    });
    
    inviter.frozenEarnings = (inviter.frozenEarnings || 0) + amount;
    await inviter.save();

    const description = `[冻结] ${rewardName} - 好友: ${friend.email || friend.name || friend._id} (等待实名)`;
    await this._createTransaction(inviter._id, amount, 'friend_order_reward', description, orderId, {
      friendId: friend._id,
      orderNumber: orderNumber,
      frozen: true
   ...
   * 解冻奖励（好友实名后调用）
   */
  static async _unfreezeRewards(friendId) {
    const inviter = await User.findOne({
      'frozenRewards.friendId': friendId
    });

    if (!inviter || !inviter.frozenRewards || inviter.frozenRewards.length === 0) {
      return;
    }

    const toUnfreeze = inviter.frozenRewards.filter(r => 
      r.friendId.toString() === friendId.toString()
    );

    if (toUnfreeze.length === 0) return;

    let totalAmount = 0;
    for (const reward of toUnfreeze) {
      totalAmount += reward.amount;
      
      const description = `[解冻] ${reward.description} - 好友已实名`;
      await this._createTransaction(
        ...
        description, 
        reward.orderId,
        { friendId: friendId, unfrozen: true }
      );
    }
    ...
    inviter.inviteEarnings.totalFromInvite = (inviter.inviteEarnings.totalFromInvite || 0) + totalAmount;
    inviter.pendingEarnings = (inviter.pendingEarnings || 0) + totalAmount;
    inviter.frozenEarnings = (inviter.frozenEarnings || 0) - totalAmount;
    
    inviter.frozenRewards = inviter.frozenRewards.filter(r => 
      r.friendId.toString() !== friendId.toString()
    );
    
    await inviter.save();

    console.log(`[FriendReward] 解冻奖励 ¥${totalAmount} 给邀请人 ${inviter._id}`);
  }
   ...
   */
  static async revokeFriendRewards(friendId, reason) {
    const friend = await User.findById(friendId);
    if (!friend) throw new NotFoundError('好友不存在');

    if (!friend.inviterId) {
      throw new BadRequestError('该用户没有邀请人');
    }

    const inviter = await User.findById(friend.inviterId);
    ...
    const rewardTransactions = await Transaction.find({
      ...
      'metadata.friendId': friendId.toString()
    });

    // 2. 计算需要撤回的总金额
    let totalRevokeAmount = 0;
    const revokedTransactions = [];

    for (const t of rewardTransactions) {
      // 跳过已撤回的记录
      if (t.metadata?.revoked) continue;
      
      // 跳过冻结状态的记录（冻结的奖励还没发放）
      if (t.description?.includes('[冻结]')) {
        // 从冻结记录中移除
        inviter.frozenRewards = inviter.frozenRewards.filter(r => 
          r.friendId.toString() !== friendId.toString()
        );
        inviter.frozenEarnings = (inviter.frozenEarnings || 0) - t.amount;
        continue;
      }
      
      // 跳过解冻记录（已经计入解冻金额）
      if (t.description?.includes('[解冻]')) continue;
      
      totalRevokeAmount += t.amount;
      revokedTransactions.push(t._id);
      
      // 标记交易为已撤回
      t.metadata = t.metadata || {};
      t.metadata.revoked = true;
      t.metadata.revokeReason = reason;
      t.metadata.revokedAt = new Date();
      await t.save();
    }
    ...
    if (totalRevokeAmount > 0) {
      // 优先从待提现余额扣除
      if (inviter.pendingEarnings >= totalRevokeAmount) {
        inviter.pendingEarnings -= totalRevokeAmount;
      } else {
        // 待提现不够，从余额扣除
        const remaining = totalRevokeAmount - inviter.pendingEarnings;
        inviter.pendingEarnings = 0;
        inviter.balance = Math.max(0, inviter.balance - remaining);
      }

      // 更新邀请收益统计
      inviter.inviteEarnings.totalFromInvite = Math.max(0, (inviter.inviteEarnings.totalFromInvite || 0) - totalRevokeAmount);
      
      // 创建撤回交易记录
      await Transaction.create({
        userId: inviter._id,
        type: 'other',
        amount: -totalRevokeAmount,
        description: `[管理员撤回] 好友 ${friend.email || friend.name || friendId} 的奖励 - ${reason}`,
        status: 'completed',
        metadata: {
          revokedFriendId: friendId,
          revokeReason: reason,
    ...
    // 4. 重置好友的订单进度
    const originalOrderCount = friend.personalOrderCount || 0;
    friend.personalOrderCount = 0;
    friend.newbieRewards = {
      kycReward: false,
      notificationReward: false,
      paymentReward: false,
      firstOrderReward: false,
      secondOrderReward: false,
      orderCount: 0,
      totalEarned: 0
    };
    
    // 更新有效成员状态
    friend.isValidMember = false;
    await friend.save();

    // 5. 更新邀请人的团队统计
    inviter.teamOrderCount = Math.max(0, (inviter.teamOrderCount || 0) - originalOrderCount);
    inviter.validDirectCount = Math.max(0, (inviter.validDirectCount || 0) - (friend.isValidMember ? 1 : 0));
    await inviter.save();

    // 6. 更新邀请人的有效团队人数
    await this._recalculateValidTeamCount(inviter._id);

    console.log(`[Admin] 撤回好友 ${friendId} 的奖励，总额 ¥${totalRevokeAmount}，原因: ${reason}`);

    return {
      success: true,
      revokedAmount: totalRevokeAmount,
      revokedTransactions: revokedTransactions.length,
      message: `已撤回 ¥${totalRevokeAmount} 奖励`
    };
  }
   ...
   * @param {string} reason - 撤回原因
   */
  static async revokeOrderReward(orderId, reason) {
    const order = await Order.findById(orderId);
    if (!order) throw new NotFoundError('订单不存在');

    const friendId = order.userId;
    const friend = await User.findById(friendId);
    if (!friend || !friend.inviterId) {
      throw new BadRequestError('该订单用户没有邀请人');
    }
    ...
    // 查找该订单产生的奖励
    const rewardTransaction = await Transaction.findOne({
      userId: inviter._id,
      orderId: orderId,
      type: 'friend_order_reward'
    });
    ...
    if (!rewardTransaction) {
      throw new BadRequestError('未找到该订单的奖励记录');
    }

    if (rewardTransaction.metadata?.revoked) {
      throw new BadRequestError('该订单奖励已被撤回');
    }

    const revokeAmount = rewardTransaction.amount;

    // 从邀请人余额中扣除
    if (inviter.pendingEarnings >= revokeAmount) {
      inviter.pendingEarnings -= revokeAmount;
    } else {
      const remaining = revokeAmount - inviter.pendingEarnings;
      inviter.pendingEarnings = 0;
      inviter.balance = Math.max(0, inviter.balance - remaining);
    }

    inviter.inviteEarnings.totalFromInvite = Math.max(0, (inviter.inviteEarnings.totalFromInvite || 0) - revokeAmount);
    await inviter.save();

    // 标记原交易为已撤回
    rewardTransaction.metadata = rewardTransaction.metadata || {};
    rewardTransaction.metadata.revoked = true;
    rewardTransaction.metadata.revokeReason = reason;
    rewardTransaction.metadata.revokedAt = new Date();
    ...
    await Transaction.create({
      ...
      type: 'other',
      amount: -revokeAmount,
      description: `[管理员撤回] 订单 ${order.orderNumber} 的奖励 - ${reason}`,
      status: 'completed'
    });

    console.log(`[Admin] 撤回订单 ${orderId} 的奖励 ¥${revokeAmount}`);

    return {
      success: true,
      revokedAmount: revokeAmount,
      message: `已撤回 ¥${revokeAmount} 奖励`
    };
  }
   ...
   * 重新计算有效团队人数
   */
  static async _recalculateValidTeamCount(leaderId) {
    const leader = await User.findById(leaderId);
    if (!leader) return;

    // 直推有效人数
    const validDirectCount = await User.countDocuments({
      inviterId: leaderId,
      isValidMember: true
    });

    // 直推用户ID列表
    const directUsers = await User.find({ inviterId: leaderId }).select('_id');
    const directIds = directUsers.map(u => u._id);

    // 间推有效人数
    const validIndirectCount = await User.countDocuments({
      inviterId: { $in: directIds },
      isValidMember: true
    });

    leader.validDirectCount = validDirectCount;
    leader.validTeamCount = validDirectCount + validIndirectCount;
    await leader.save();
  }
   ...
   * 获取好友的奖励历史（管理员用）
   */
  static async getFriendRewardHistory(friendId) {
    const friend = await User.findById(friendId);
    if (!friend) throw new NotFoundError('好友不存在');

    if (!friend.inviterId) {
      return { friend, inviter: null, rewards: [], frozenRewards: [] };
    }
    ...
    const rewards = await Transaction.find({
      userId: friend.inviterId,
      type: 'friend_order_reward',
      $or: [
        { 'metadata.friendId': friendId.toString() },
        { description: { $regex: friend.email || friend.name || friendId, $options: 'i' } }
      ]
    }).sort({ createdAt: -1 });

    // 获取冻结的奖励
    const frozenRewards = inviter?.frozenRewards?.filter(r => 
      r.friendId.toString() === friendId.toString()
    ) || [];
    ...
    return {
      ...
      rewards,
      frozenRewards,
      totalRewarded: rewards.filter(r => !r.metadata?.revoked && !r.description?.includes('[冻结]')).reduce((sum, r) => sum + r.amount, 0),
      totalFrozen: frozenRewards.reduce((sum, r) => sum + r.amount, 0),
      totalRevoked: rewards.filter(r => r.metadata?.revoked).reduce((sum, r) => sum + r.amount, 0)
    };
  }
   ...
   * 用户实名认证成功后调用
   */
  static async onKycVerified(userId) {
    const user = await User.findById(userId);
    if (!user) return;

    console.log(`[Team] 用户 ${userId} 实名认证成功`);

    await this.updateValidMemberStatus(userId);
    await this._unfreezeRewards(userId);
  }
  ...
  static async _createTransaction(userId, amount, type, description, orderId = null, metadata = null) {
    const user = await User.findById(userId);
  ...
  static async getTeamStats(userId) {
    ...
    commissions.forEach(t => {
      ...
      // 跳过已撤回的
      if (t.metadata?.revoked) return;
      // 跳过冻结的（负数）
      if (t.description?.includes('[冻结]')) return;
    ...
    return {
      ...
      frozenEarnings: user.frozenEarnings || 0,
  ...
  static async getTeamList(userId, type, keyword, page = 1, limit = 20) {
    ...
    const selectFields = '_id name email phone avatar createdAt balance isValidMember kycStatus personalOrderCount totalCommission newbieRewards';
  ...
  static async getInviteTaskInfo(userId) {
    ...
    const completedReward = inviteTransactions
      .filter(t => !t.metadata?.revoked && !t.description?.includes('[冻结]'))
  ...
  static async getFriendCommissions(userId, friendId) {
    ...
    return friendCommissions.map(t => ({
      ...
      revoked: t.metadata?.revoked || false
    }));
  }
  ...
  static async getTeamLeaderList(filters = {}, page = 1, limit = 20) {
    ...
    const leaders = await User.find(query)
      .select('name email agentRank validDirectCount validTeamCount teamOrderCount ' +
              'inviteEarnings teamStats performanceRewards frozenEarnings frozenRewards createdAt')
  ...
  static async getTeamLeaderDetail(leaderId) {
    ...
    const directMembers = await User.find({ inviterId: leaderId })
      .select('_id name email kycStatus isValidMember personalOrderCount createdAt')
      .sort({ createdAt: -1 });
