...
export class UserService {
  ...
  static async exchangePointsForCoins(userId, points, coinsRate = 100) {
    ...
    if (coinsToReceive <= 0) throw new BadRequestError('积分不足以兑换1个小象币');
    ...
    const user = await User.findOneAndUpdate(
      ...
      {
        $inc: { points: -points, coins: coinsToReceive }
      },
      { new: true }
    );
    ...
    await Transaction.create([{
      ...
      amount: points,
      balanceSnapshot: user.points,
      description: `积分兑换小象币：${points}积分 → ${coinsToReceive}小象币`,
      status: 'completed'
    }, {
      userId: user._id,
      type: 'coins_exchange',
      ...
      balanceSnapshot: user.coins,
      description: `积分兑换获得：${coinsToReceive}小象币`,
      status: 'completed'
    }]);

    clearCache('/api/users/profile');
    console.log(`[UserService] 积分兑换成功: 用户 ${user.email}, ${points}积分 → ${coinsToReceive}小象币`);
    return { user, pointsSpent: points, coinsReceived: coinsToReceive };
  }
