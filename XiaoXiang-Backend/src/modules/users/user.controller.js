...
export class UserController {
   ...
   * 前端轮询此接口获取审核结果
   */
  static getNameStatus = asyncHandler(async (req, res) => {
    const userId = req.user._id;
    const status = await UserServiceProfile.getNameStatus(userId);
    return success(res, status, '获取改名状态成功');
  });
  ...
  static updateMyProfile = asyncHandler(async (req, res) => {
    ...
    // 处理昵称修改（走审核流程）
    if (name) {
      const result = await UserServiceProfile.requestNameChange(userId, name);
      
      // 如果还有头像要修改，一并处理
      if (avatarColor) {
        await UserServiceProfile.updateAvatarColor(userId, avatarColor);
      }
      
      return success(res, {
        status: result.status,
        pendingName: result.pendingName,
        avatarColor: avatarColor || (await User.findById(userId)).avatarColor,
    ...
    // 只修改头像（直接生效）
    if (avatarColor) {
      const result = await UserServiceProfile.updateAvatarColor(userId, avatarColor);
      return success(res, {
        avatarColor: result.avatarColor
      }, '头像修改成功');
    }

    return success(res, {}, '无修改');
  });

  /**
   * 获取审核队列状态（管理员接口）
   */
  static getReviewQueueStatus = asyncHandler(async (req, res) => {
    const status = UserServiceProfile.getQueueStatus();
    return success(res, status, '获取队列状态成功');
  });

  /**
   * 手动触发审核队列处理（管理员接口）
   */
  static triggerReviewQueue = asyncHandler(async (req, res) => {
    const result = UserServiceProfile.triggerQueueProcess();
    return success(res, result, '已触发队列处理');
  });
}
