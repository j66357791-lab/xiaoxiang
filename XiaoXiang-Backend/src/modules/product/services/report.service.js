...
export const generateDailyReport = async (user) => {
  ...
  const orderStats = { total: orders.length, completed: orders.filter(o => o.status === 'completed').length, cancelled: orders.filter(o => o.status === 'cancelled').length };
  ...
  const stockChanges = stockLogs.map(log => ({ productId: log.targetId?._id, productName: log.targetId?.name, change: log.changes.diff, reason: log.reason }));
  const report = new DailyReport({ reportDate: yesterday, generatedBy: user._id, orderStats, financialStats, stockChanges });
  await report.save();
  return report;
};
...
export const exportReport = async (reportId) => {
  ...
  const headers = ['项目', '数值'];
  const rows = [
    ['日期', new Date(report.reportDate).toLocaleDateString()],
    ['订单总数', report.orderStats.total],
    ['完成订单', report.orderStats.completed],
    ['取消订单', report.orderStats.cancelled],
    ['总收入', report.financialStats.totalIncome],
    ['总成本', report.financialStats.totalCost],
    ['总利润', report.financialStats.totalProfit],
    ['已结算', report.financialStats.totalSettled],
    ['待结算', report.financialStats.pendingSettle],
  ];
  return { headers, rows };
};
