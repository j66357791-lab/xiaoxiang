...
export class StatsService {
   ...
   * 
   * 【积分定义】
   * 发行：礼包购买、系统赠送、小象币兑换积分
   * 销毁：游戏投入、积分兑换小象币、矿池投入
   * 
   * 【小象币定义】
   * 发行：每日矿池发行（mining_reward）
   * 销毁：用户主动兑换积分（mining_exchange）、转增手续费（coins_transfer_fee）
   */
  static async getCurrencyOverview() {
    ...
    // 小象币发行类型
    const coinsIssuedTypes = [
      'mining_reward',
    ];

    // 小象币销毁类型
    const coinsDestroyedTypes = [
      'mining_exchange',
      'coins_transfer_fee',
    ];
      ...
      { $match: filter },
      {
        $group: {
          ...
          totalIncome: {
            $sum: {
              $cond: [
                { $in: ['$type', ['points_income', 'coins_income', 'flipcard_reward', 'wheel_reward', 'wheel_jackpot', 'caiquan_reward', 'race_reward', 'mystery_shop_reward', 'gift_purchase', 'mining_reward']] },
                '$amount',
                0
              ]
            }
