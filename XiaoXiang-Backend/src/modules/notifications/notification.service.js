...
export class NotificationService {
   ...
   * @param {string} senderName - 发送方用户名
   * @param {number} amount - 转增数量
   */
  static async sendTransferNotification(receiverId, senderName, amount) {
    return this.createAndSend(
      receiverId,
      '小象币到账通知',
      `${senderName} 给您赠送了 ${amount} 小象币，已到账！`,
      'transfer',
      { action: 'view_transfer', amount }
    );
  }
}
