 ...
 * 更新FAQ分类
 */
export async function updateFAQCategory(req, res) {
  try {
    const { categoryId } = req.params;
    const category = await helpService.updateFAQCategory(categoryId, req.body);
    if (!category) {
      return res.status(404).json({ code: 404, message: '分类不存在' });
    }
    res.json({ code: 200, message: '更新成功', data: category });
  } catch (error) {
    console.error('[更新FAQ分类] 错误:', error);
 ...
 * 删除FAQ分类
 */
export async function deleteFAQCategory(req, res) {
  try {
    const { categoryId } = req.params;
    const result = await helpService.deleteFAQCategory(categoryId);
    if (!result) {
      return res.status(404).json({ code: 404, message: '分类不存在' });
    }
    res.json({ code: 200, message: '删除成功' });
  } catch (error) {
    console.error('[删除FAQ分类] 错误:', error);
...
export async function createFAQFlow(req, res) {
  try {
    ...
    res.json({ code: 200, message: '创建成功', data: flow });
  } catch (error) {
 ...
 * 更新FAQ流程
 */
export async function updateFAQFlow(req, res) {
  try {
    const { flowId } = req.params;
    const flow = await helpService.updateFAQFlow(flowId, req.body);
    if (!flow) {
      return res.status(404).json({ code: 404, message: '流程不存在' });
    }
    res.json({ code: 200, message: '更新成功', data: flow });
  } catch (error) {
    console.error('[更新FAQ流程] 错误:', error);
    res.status(500).json({ code: 500, message: error.message || '更新失败' });
  }
}
 ...
 * 删除FAQ流程
 */
export async function deleteFAQFlow(req, res) {
  try {
    const { flowId } = req.params;
    const result = await helpService.deleteFAQFlow(flowId);
    if (!result) {
      return res.status(404).json({ code: 404, message: '流程不存在' });
    }
    res.json({ code: 200, message: '删除成功' });
  } catch (error) {
    console.error('[删除FAQ流程] 错误:', error);
    res.status(500).json({ code: 500, message: error.message || '删除失败' });
  }
}
 ...
 * 初始化默认FAQ数据
 */
export async function initDefaultFAQ(req, res) {
  try {
    const result = await helpService.initDefaultFAQ();
    res.json({ code: 200, message: 'success', data: result });
  } catch (error) {
    console.error('[初始化FAQ] 错误:', error);
