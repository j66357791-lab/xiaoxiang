...
// 客服服务层 - 优化版（支持客服分流、自动接单、工单详情持久化、服务历史）

import ZAI from 'z-ai-web-dev-sdk';
import models from './help-people.model.js';

const { Ticket, Message, FAQ, ServiceStaff, QuickReply, ServiceHistory } = models;
 ...
 * 管理员创建工单（新增功能）
 */
export async function adminCreateTicket(userId, userInfo = {}, issueType = 'complaint', description = '', serviceId = '001') {
  // 检查是否有未关闭的工单
  const existingTicket = await Ticket.findOne({
    userId,
    status: { $in: ['ai_chatting', 'pending', 'in_progress', 'waiting_user'] },
  ...
  if (existingTicket) {
    throw new Error('该用户有未关闭的工单，请先处理现有工单');
  }
  ...
  const title = description ? description.substring(0, 50) : '新工单';
  ...
  await Ticket.updateOne(
    { ticketId: ticket.ticketId },
    {
      ...
      $addToSet: { 'assignedTo.activeTicketIds': ticket.ticketId },
    }
  );
  ...
  await ServiceStaff.updateOne(
    { serviceId },
    {
      $addToSet: { activeTicketIds: ticket.ticketId },
      updatedAt: new Date(),
    }
  );
  ...
  await ServiceHistory.create({
    userId,
    ticketId: ticket.ticketId,
    issueType: getIssueTypeText(issueType),
    progress: `${staff.nickname}客服处理中`,
    serviceInfo: {
      serviceId: staff.serviceId,
      nickname: staff.nickname,
  ...
  const messages = await Message.find({ ticketId: ticket.ticketId })
    .sort({ createdAt: 1 })
    .lean();
...
export async function saveTicketDetail(ticketId, detailData, serviceId = '001') {
  ...
  if (result.modifiedCount > 0) {
    ...
    await ServiceHistory.findOneAndUpdate(
      { ticketId },
      {
        issueType: getIssueTypeText(detailData.issueType),
        updatedAt: new Date(),
      }
    );
