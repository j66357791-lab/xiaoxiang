...
export class OrderService {
  ...
  static async applyForJob(userId, jobId, levelIndex = 0) {
    ...
    const order = await Order.create({
      ...
      rewardPoints: job.rewardPoints || 0,
  ...
  static async updateOrderStatus(orderId, status, options = {}) {
    ...
    const allowedTransitions = {
      'Applied': ['Submitted', 'Cancelled', 'Reviewing', 'PendingPayment', 'Completed', 'Rejected'],
      'Submitted': ['Reviewing', 'PendingPayment', 'Completed', 'Rejected', 'Cancelled'],
      'Reviewing': ['PendingPayment', 'Completed', 'Rejected', 'Cancelled'],
      ...
      case 'Completed':
        if (order.status !== 'Completed') {
          ...
          try {
            ...
            // 4. 发放赠送积分
            if (order.rewardPoints > 0 && !order.rewardPointsIssued) {
              const worker = await User.findById(targetUserId);
              if (worker) {
                worker.points += order.rewardPoints;
                await worker.save();
                ...
                await Transaction.create({
                  ...
                  pointsSnapshot: worker.points,
                  description: `订单完成奖励：${order.jobSnapshot.title}`,
                  orderId: order._id,
                  status: 'completed'
                });
                
                order.rewardPointsIssued = true;
                console.log(`[OrderService] 🎁 发放赠送积分 ${order.rewardPoints} 给用户 ${targetUserId}`);
              }
            }
            ...
            try {
              ...
              await AssetService.createAssetFromOrder(order);
              console.log(`[OrderService] 📦 订单 ${order.orderNumber} 已自动生成资产记录`);
            } catch (assetErr) {
              console.error('[OrderService] 自动创建资产失败 (不影响订单流程):', assetErr);
            }
            // ===============================================================
  ...
  static async getOrderStats(timeRange = 'today') {
    ...
    stats.forEach(stat => {
      formattedStats[stat._id] = {
        ...
        rewardPoints: stat.totalRewardPoints || 0
      };
      totalOrders += stat.count;
      totalAmount += stat.totalAmount;
      totalRewardPoints += stat.totalRewardPoints || 0;
    });

    const allStatuses = ['Applied', 'Submitted', 'Reviewing', 'PendingPayment', 'Completed', 'Cancelled', 'Rejected'];
    allStatuses.forEach(status => {
      if (!formattedStats[status]) {
        formattedStats[status] = { count: 0, amount: 0, rewardPoints: 0 };
      }
    });
