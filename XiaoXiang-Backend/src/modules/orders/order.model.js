...
const OrderSchema = new mongoose.Schema({
  ...
  // ==================== 价格信息 ====================
  pricing: {
    estimatedPrice: { type: Number },      // 预估价格
    quotedPrice: { type: Number },         // 报价价格
    finalPrice: { type: Number },          // 最终价格
    priceFactors: [{
      factor: { type: String },
      impact: { type: Number },
      description: { type: String },
  ...
  pickupInfo: {
    ...
    notes: String,
    collectorId: { type: mongoose.Schema.Types.ObjectId, ref: 'User' }, // 收件员
  },
  ...
  // ==================== 🆕 订单确认信息 ====================
  confirmation: {
    condition: String,          // 确认的成色
    description: String,        // 商品描述
    shippingMethod: String,     // 寄送方式
    estimatedPrice: Number,     // 确认的预估价格
    estimatedPaymentTime: Date, // 预估打款时间
    couponApplied: { type: Boolean, default: false }, // 是否使用优惠券
...
OrderSchema.index({ 'coupon.id': 1 });
OrderSchema.index({ 'warehouse.id': 1 });
...
OrderSchema.methods.calculateFinalPrice = function() {
  let finalPrice = this.pricing.quotedPrice || this.pricing.estimatedPrice || 0;
...
// 换绑仓库
OrderSchema.methods.changeWarehouse = function(newWarehouse, reason, changedBy) {
  // 记录历史
  this.warehouseHistory.push({
    fromWarehouse: {
  ...
  this.warehouse = {
    id: newWarehouse._id,
    name: newWarehouse.name,
    address: newWarehouse.address?.detail,
    phone: newWarehouse.contact?.phone,
  };
};
