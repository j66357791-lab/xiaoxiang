...
export class FlipCardService {
  ...
  /**
   * 验证前端翻牌数据是否合法
   * @param {Array} cards - 后端存储的卡片布局
   * @param {Array} flippedCards - 前端传递的翻牌索引数组
   * @param {Array} turtlePositions - 后端存储的乌龟位置
   * @returns {Object} - 验证结果 { valid: boolean, flippedRabbits: number, hitTurtle: boolean }
   */
  static validateFlippedCards(cards, flippedCards, turtlePositions) {
    let flippedRabbits = 0;
    let hitTurtle = false;

    for (const index of flippedCards) {
      if (index < 0 || index >= cards.length) {
        return { valid: false, error: '无效的卡片索引' };
      }
      
      const cardType = cards[index].type;
      if (cardType === 'turtle') {
        hitTurtle = true;
      } else if (cardType === 'rabbit') {
        flippedRabbits++;
      }
    }

    return { valid: true, flippedRabbits, hitTurtle };
  }

  /**
   * 计算当前分数
   * @param {number} ticketPrice - 门票金额
   * @param {number} factor - 倍率
   * @param {number} flippedRabbits - 已翻兔子数量
   * @returns {number} - 当前分数
   */
  static calculateScore(ticketPrice, factor, flippedRabbits) {
    let score = ticketPrice;
    for (let i = 0; i < flippedRabbits; i++) {
      score = Math.floor(score * factor * 100) / 100;
    }
    return score;
  }
   ...
   * @param {number} clientScore - 前端传递的当前分数
   * @param {Array} clientFlippedCards - 前端传递的翻牌记录（数组，1表示已翻开）
   * @returns {Object} - 结算结果
   */
  static async settleGame(userId, gameId, clientScore = null, clientFlippedCards = null) {
    const game = await FlipCardGame.findById(gameId);
    ...
    if (clientFlippedCards && Array.isArray(clientFlippedCards)) {
      // 前端传递的是 [0,1,0,1,...] 格式，转换为索引数组
      clientFlippedCards.forEach((flipped, index) => {
        if (flipped === 1 && index < game.cards.length) {
    ...
    // 🆕 验证翻牌数据
    const validation = this.validateFlippedCards(
      game.cards, 
      flippedCardIndexes, 
      game.cards.map((c, i) => c.type === 'turtle' ? i : -1).filter(i => i !== -1)
    );

    if (!validation.valid) {
      throw new BadRequestError(validation.error || '翻牌数据验证失败');
    }

    // 🆕 如果翻到了乌龟，不能结算（应该已经输了）
    if (validation.hitTurtle) {
      // 更新游戏状态为失败
      game.status = 'lost';
      game.currentScore = 0;
      game.finalReward = 0;
      game.fee = 0;
      game.flippedRabbits = validation.flippedRabbits;
      flippedCardIndexes.forEach(index => {
        game.cards[index].flipped = true;
        if (!game.flippedCards.includes(index)) {
          game.flippedCards.push(index);
        }
      ...
      throw new BadRequestError('您已翻到乌龟，游戏失败');
    }

    // 🆕 检查是否至少翻了一张兔子
    if (validation.flippedRabbits === 0) {
      throw new BadRequestError('请先翻牌再结算');
    }

    // 🆕 计算后端验证的分数
    const serverScore = this.calculateScore(game.ticketPrice, game.factor, validation.flippedRabbits);

    // 🆕 校对前端分数（允许小数误差）
    const scoreDiff = Math.abs((clientScore || 0) - serverScore);
    if (clientScore !== null && scoreDiff > 0.1) {
      console.log(`[FlipCard] 分数校对差异: 前端=${clientScore}, 后端=${serverScore}, 差异=${scoreDiff}`);
      // 使用后端计算的分数为准
    }

    // 使用后端验证后的分数
    const totalScore = serverScore;

    // 🆕 检查是否翻完所有兔子（自动胜利）
    if (validation.flippedRabbits >= game.rabbitCount) {
      // 翻完所有兔子，自动胜利
      const session = await mongoose.startSession();
      session.startTransaction();
      ...
      try {
        ...
        console.log(`[FlipCard] 用户翻完所有兔子，自动胜利！获得 ${actualReward} 积分`);
