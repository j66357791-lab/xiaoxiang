...
export class FlipCardController {
  ...
  static settle = asyncHandler(async (req, res) => {
    const { gameId, currentScore, flippedCards } = req.body;
    ...
    // 🆕 接受前端传递的翻牌数据
    const result = await FlipCardService.settleGame(
      req.user.id, 
      gameId, 
      currentScore || null,      // 前端计算的分数
      flippedCards || null       // 前端翻牌记录 [0,1,0,1,...]
    );
    
    // 返回结算详情
    return success(res, {
      message: result.message || '结算成功',
      totalScore: result.totalScore,      // 获得积分
      fee: result.fee,                    // 手续费
      actualReward: result.actualReward,  // 实际获得
      newBalance: result.newBalance,
      isWin: result.isWin || false,       // 是否自动胜利
      fullLayout: result.fullLayout       // 完整布局（可选）
    }, result.message || '结算成功');
  });
