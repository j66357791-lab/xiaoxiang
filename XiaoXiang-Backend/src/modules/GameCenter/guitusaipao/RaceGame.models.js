...
const raceHistorySchema = new mongoose.Schema({
  raceId: { type: String, required: true, unique: true },
  raceNumber: { type: Number, required: true, index: true },
  winner: { type: String, enum: ['turtle', 'rabbit'], required: true },
  turtlePosition: { type: Number, default: 0 },
  rabbitPosition: { type: Number, default: 0 },
...
// 创建索引以加速查询
raceHistorySchema.index({ raceNumber: -1 });
raceHistorySchema.index({ winner: 1 });
